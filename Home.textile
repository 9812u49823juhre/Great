...
A lot of people ask if they can help code up DuckDuckGo? Yes, yes, a thousand times yes! 

But how?  This document attempts to answer that question.
...
Most of DuckDuckGo is closed source, but a growing portion is "open":http://github.com/duckduckgo. We don't think it is right to ask volunteers to contribute to closed source, so what we propose here will also be open. However, if you want to work on the closed portion, there is opportunity for that as well. It's just a bit more complicated.

In any case, if after reading this doc you are at all interested, please contact us at "open@duckduckgo.com":mailto:open@duckduckgo.com. _And please, please make contact before you get started so that we can ensure the shortest path to having what you do go live._

The main area we could use help with is *Zero-click Info.* That's the stuff that appears above the traditional links and provides instant answers, topic summaries, etc. Basically, we want more of it and we want what we have to be much better!
...
* *Getting data dumps.* A lot of our zero-click info comes from essentially static data dumps, usually achieved through Web crawling (but could also originate from another data dump from the underlying provider). For example, our "programming documentation":http://duckduckgo.com/?q=perl+split mainly comes from crawling done by "@boyter":http://twitter.com/boyter. Two open examples are "launchpad":https://github.com/duckduckgo/Launchpad made by "@crazedpsyc":http://twitter.com/crazedpsyc and "lyricsmode":https://github.com/duckduckgo/lyrics made by "@dhruvbird". 
-
This category of 0-click involves getting the data (through crawling or from a dump) and then putting it in a form we can use. You could really do it in any language. Most of our stuff internally is in Perl, and the above examples are in Python.
-
We should note that this is a win-win for the data provider since we only show a small snippet and then link back to them at the very top of the results. However, it is often a good idea to OK it with the provider ahead of time. That is also useful because sometimes they will provide you with data dumps. Otherwise, make sure you crawl at a very slow rate amd/or use open sources.
...
-
We are working on an open example, but in the mean time, if this interests you just ask and we will show you how to do it. There are both standard functions to call for standard display and then there are cases where the JS creates the HTML on the fly and it gets inserted into the right div. 
-
